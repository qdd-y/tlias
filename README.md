...
这是一份给联调和运维同学的快速说明。 当前前端静态资源已经打包，运行在 Nginx 容器里，后端为 Spring Boot 容器。
...
这个现象通常是请求没有经过 Docker 里的 Nginx，而是走了本地 Vite 开发服务器（5173）。

优先按下面顺序排查：

你访问的是不是 http://localhost:18080（容器 Nginx）而不是 http://localhost:5173（Vite dev server）。
如果必须用 5173 开发模式，确认 Vite 是否配置了 /api 代理到后端。
登录接口路径是否与后端一致（例如 /api/login 或 /api/emps/login，以实际后端路由为准）。
登录后请求是否带上 token（一般放在请求头）。
5. 联调建议
